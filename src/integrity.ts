...
export async function checkImageSources() {
  ...
  if (config.REMOVE_DANGLING_FILE_REFERENCES === true)
    ...
    logger.warn(
      `REMOVE_DANGLING_FILE_REFERENCES would have removed ${removedReferences} images. However, the changes are not committed, because the options is disabled in config.json.`
    );
