...
export default class Scene {
    ...
    static async generatePreview(scene: Scene): Promise<string | null> {
        return new Promise(async (resolve, reject) => {
            ...
            const options = {
                file: scene.path,
                pattern: `${scene._id}.jpg`,
                count: 100,
                thumbnailPath: await libraryPath("previews/"),
                quality: "60"
            };

            const tmp = path.join(os.tmpdir(), "dplayer-thumbnails", scene._id);
                        ...
                        if (err) {
                            ...
                            Jimp.read(tmp + "/sprite.png", function (err, lenna) {
                                if (err) {
                                    ...
                                    lenna.quality(parseInt(options.quality)).write(path.join(options.thumbnailPath, options.pattern));
                                    rm(tmp, function () {
                                        logger.log("Finished generating preview");
                                        resolve(path.join(options.thumbnailPath, options.pattern));
                                    });
                                }
                            });
                        }
