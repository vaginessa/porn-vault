<template>
  <v-card tile v-if="studio">
    ...
    <div v-if="studio.parent" class="mt-2 pl-4 text-uppercase caption">
      <router-link
        class="hover"
    ...
    <v-card-title
      :class="`pb-4 ${studio.parent ? 'pt-0' : 'pt-2'}`"
    ...
    <v-card-subtitle :class="`py-0 ${!studio.numScenes ? 'mb-2' : ''}`"
      >{{ studio.numScenes }} {{ studio.numScenes == 1 ? "scene" : "scenes" }}</v-card-subtitle
    >
    <v-card-text class="pt-1 pb-2" v-if="studio.labels.length && showLabels">
